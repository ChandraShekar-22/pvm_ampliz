...
export class SelectBulkModalComponent implements OnInit {
  ...
  selectListFromAuto(list: MatAutocompleteSelectedEvent) {
    console.log(list.option.value, "Value Auto");
      ...
      this.b2bService.savePeopleAsPerCount(body).subscribe(
        (res) => {
          setTimeout(() => {
            ...
            this.messageService.display(
              true,
              `Successfully saved ${this.numberOfContacts} contacts `
            );
  ...
  handleContactFocus(event) {
    this.numberOfContacts =
      this.numberOfContacts != 0 ? this.numberOfContacts : null;
  }
  handleContactBlur(event) {
    this.numberOfContacts =
      this.numberOfContacts != null ? this.numberOfContacts : 0;
  }
