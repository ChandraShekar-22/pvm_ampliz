...
export class FilterImagingCenterExecutivesComponent implements OnInit {
  ...
  getPersistData() {
    // let that = this;
    setTimeout(() => {
      this.filterData.company = this.filterStorageService.get(
        "imaging_includedCompanyList"
      ) || [];
      this.filterData.fullName = this.filterStorageService.get(
        "imaging_includedContactsList"
      ) || [];
      this.filterData.titleInclude = this.filterStorageService.get(
        "imaging_includedTitleList"
      ) || [];
      this.filterData.titleExclude = this.filterStorageService.get(
        "imaging_excludedTitleList"
      ) || [];
      this.filterData.industryInclude = this.filterStorageService.get(
        "imaging_includedIndustryList"
      ) || [];
      this.filterData.titleExclude = this.filterStorageService.get(
        "imaging_excludedIndustryList"
      ) || [];
      this.filterData.seniority = this.filterStorageService.get(
        "imaging_includedSeniorityList"
      ) || [];
      this.filterData.department = this.filterStorageService.get(
        "imaging_includedDepartmentList"
      ) || [];

      this.filterData.stateList = this.filterStorageService.get(
        "imaging_selectedStates"
      ) || [];
      this.filterData.cityList = this.filterStorageService.get(
        "imaging_selectedCities"
      ) || [];
      this.selectedCountry = this.filterStorageService.get(
        "imaging_selectedCountry"
      ) || [];
      this.filterData.numberOfImagingEquipments = this.filterStorageService.getNumber(
