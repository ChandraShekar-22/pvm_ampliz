...
export class FilterImagingCenterComponent implements OnInit,OnDestroy {
  ...
  getPersistData() {
    // let that = this;
    setTimeout(() => {
      this.filterData.centerName = this.filterStorageService.get(
        "imaging_centerName"
      ) || [];
      ...
      this.filterData.noOfEqupment = this.filterStorageService.getNumber(
        "imaging_noOfEqupment"
      ) || 0;
      // setTimeout(() => {
      this.omitChanges();
    });
