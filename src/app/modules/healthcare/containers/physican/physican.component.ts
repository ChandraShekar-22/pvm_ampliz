...
export class PhysicanPageComponent implements OnInit, AfterViewInit {
  ...
  unmaskSavedPhysicians() {
    this.dataService.savedPhysicians.subscribe((res: Array<any>) => {
      ...
      res.map((savedPhysician: any) => {
        const index = this.totalSearchResult.findIndex(physicianItem => savedPhysician.physicianId == physicianItem.physicianId);
        if (index !== -1) {
  ...
  get showSaveAllCheckbox() {
    return ((this.selectedPhysicianInCurrentPage.length == 0
  ...
  get showSaveAllPArtial() {
    return (this.selectedPhysicianInCurrentPage.length > 0 && this.selectedPhysicianInCurrentPage.length < this.totalSearchResult.length)
  }
  ...
  handleChange(value, model) {
    ...
    if (model === "leadWithEmail") {
      ...
      this.amplizService.searchPhysician(this.physicianSearchParameter).subscribe((res) => {
        this.loaderService.display(false);
        ...
        if (this.totalSearchResult.length !== 0) {
          this.noResult = false;
          this.selectedPhysicianInCurrentPage = this.totalSearchResult.filter((contactItem: any) => {
            return this.selectedPhysician.includes(contactItem.physicianId)
          }).map((item: any) => item.physicianId);
          this.savedPhysicians = this.totalSearchResult.filter((contactItem: any) => { return contactItem.leadSaveStatus == 'Saved' });
          this.totalCount = res.totalResult;
          if (this.tab === 1) {
  ...
  setPhysicianSearchParams() {
    ...
    this.physicianSearchParameter.savedListOffset = this.previousSavedOffsets[this.previousSavedOffsets.length - 1] || 0;
  ...
  checkSelectAll(val) {
    this.selectedPhysician = this.selectedPhysician.filter(item => !this.selectedPhysicianInCurrentPage.includes(item));
    this.selectedPhysicianInCurrentPage = [];
    if (val == true) {
