...
import {
  PayorExecutive,
  PayorExecutiveList,
} from "../../models/payor-executive.model";
...
export class PayorExecutiveComponent implements OnInit {
  ...
  get isPartiallySelected() {
    const count =
      this.selectedFilter.searchType == "TOTAL"
        ...
        : this.netNewCount;
    const limit =
      this.selectedFilter.limit > count ? count : this.selectedFilter.limit;
    return (
      this.selectedImagingsInCurrentPage.length < limit &&
      this.selectedImagingsInCurrentPage.length > 0
    );
  }
  get allSelected() {
    const count =
  ...
  async searchImagingsList(updatePreviousOffsets: boolean = true) {
    ...
    if (updatePreviousOffsets) {
      ...
      this.selectedFilter.savedListOffset =
          ...
          this.selectedImagingsInCurrentPage =
            this.imagingList.payorExecutiveInfoList
              .filter((imagingItem: any) => {
                return this.selectedImagings.includes(
                  imagingItem.mcoExecutiveId
                );
              })
              .map((item: any) => item.mcoExecutiveId);
          this.totalItemCount = res.totalResult;
          this.changeTabItems();
          this.savedImagings = this.imagingList.payorExecutiveInfoList.filter(
            (imageItem: any) => {
  ...
  get incrementEnabled() {
    return (
      this.offset + this.selectedFilter.limit < this.totalItemCount &&
  ...
  handleCurrentCheckboxChange(imaging) {
    const currentimagingsIndex = this.selectedImagingsInCurrentPage.indexOf(
      imaging.mcoExecutiveId
    );
  ...
  handleCheckboxChangeForAll(imaging) {
    const imagingsIndex = this.selectedImagings.findIndex(
      (item) => item == imaging.mcoExecutiveId
    );
    if (imagingsIndex == -1) {
      if (imaging.leadSaveStatus !== "Saved") {
  ...
  changeTabItems() {
    ...
    this.tabItems[1].name = `Net New (${this.numberWithCommas(
      this.netNewCount
    )})`;
  }
